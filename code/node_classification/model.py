...
class GCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        x = self.lin1(x)  #torch.Size([2708, 64])
        x = F.relu(propagate(x, edge_index, edge_weight) + self.bias1) #torch.Size([2708, 64])
        x = F.dropout(x, p=self.args.dropout, training=self.training) #]torch.Size([2708, 64])
        x = self.lin2(x) #torch.Size([2708, 7])
        x = propagate(x, edge_index, edge_weight) + self.bias2 #torch.Size([2708, 7])
        return x
