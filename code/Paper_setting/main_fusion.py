...
def run(model, optimizer, train_cf, clicked_set, user_dict, adj, args):
    ...
    losses, recall, ndcg, precision, hit_ratio, F1 = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list)
    start = time.time()
    ...
    for epoch in range(args.epochs):
        ...
        dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,collate_fn=dataset.collate_batch, pin_memory=args.pin_memory)  # organzie the dataloader based on re-sampled negative pairs
        ...
        if not epoch % 5:
            model.eval()
            res = PrettyTable()
            res.field_names = ["Time", "Epoch", "Training_loss","Recall", "NDCG", "Precision", "Hit_ratio", "F1"]
            ...
            if test_res['Recall'][3] > test_recall_best:
                ...
                if args.save:
                    torch.save(model.state_dict(), os.getcwd() +'/trained_model/' + args.dataset + '/' + args.model + str(args.neg_in_val_test) + '.pkl')
